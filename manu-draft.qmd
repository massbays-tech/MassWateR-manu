...
```{r}
#| label: fig-workflow
#| fig-cap: 'Workflow demonstrating how a user could engage with the *MassWateR* package. WQX: Water Quality Exchange; QC: Quality Control.'
#| fig-alt: 'Workflow diagram showing four ways to engage with *MassWateR*'
#| out-width: '100%'
...
#| fig-cap: "Pseudocode demonstrating the iterative process of importing a required data file for *MassWateR*.  All read functions import an Excel file from the user-specified path (e.g., `respth`) and the imported file is then passed to a check function.  The function exits if an error is encountered, allowing the user to manually fix the identified error and then import again.  After all checks are passed, a formatting function is applied to correct minor issues (e.g., standardize date format as YYYY-MM-DD) and the final data object is returned (e.g., `resdat`)."
#| fig-alt: 'Diagram showing pseudocode for importing and fixing a data file with *MassWateR*'
#| out-width: "100%"
